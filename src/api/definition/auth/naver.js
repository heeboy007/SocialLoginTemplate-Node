...
async function naver(req, res) {
    logger.info("API : auth.controller.js : naver");
    ...
    try {
        ...
        naver_id = res.data.response["id"];
        email = res.data.response["email"];
        nickname = res.data.response["nickname"] || ("Tiempo" + generate6Digits());

        logger.info(JSON.stringify(res.data.response));
    } catch (e) {
    ...
    const user = await schemas.User.findOne({ 
        where: { 
            social_media_external_id: naver_id, 
            login_method: "naver",
            state: { [Op.not]: "closed" }
        }
    });
